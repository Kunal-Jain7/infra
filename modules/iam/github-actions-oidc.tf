...
resource "aws_iam_openid_connect_provider" "github_actions" {
  ...
  tags = {
    Name    = "${var.project}-${var.env}-github-actions-oidc-provider"
    Env     = var.env
    Project = var.project
  }
}
...
data "aws_iam_policy_document" "github_actions_assume_role" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "tokens.actions.githubusercontent.com:sub"
      values = [
...
resource "aws_iam_role" "github_actions_ci" {
  name                 = "${var.project}-${var.env}-github-actions-ci-role"
...
resource "aws_iam_policy" "github_actions_ci_policy" {
  name        = "${var.project}-${var.env}-github-actions-ci-policy"
